@{
    VHDXPath             = "E:\VM\Setup\VHDX\ubuntu-24.04.1-live-server-amd64.VHDX"
    # SwitchName           = "Realtek Gaming 2.5GbE Family Controller - Virtual Switch"
    # ParentVHDPath        = "D:\VM\Setup\VHDX\Windows_SERVER_2025_EVAL_x64FRE_en-us-May-17-2024-100GB.VHDX"
    VMPath               = "E:\VM"
    InstallMediaPath     = "E:\iso\ubuntu-24.04.1-live-server-amd64.iso"
    MemoryStartupBytes   = "4GB"
    MemoryMinimumBytes   = "4GB"
    MemoryMaximumBytes   = "16GB"
    Generation           = 2
    VMNamePrefixFormat   = "{0:D3} - Ubuntu - CCI-LayerNext01"
    ProcessorCount       = 28
}