...
function Get-NetworkConfigStatus {
    ...
    try {
        ...
        $networkStatus = Invoke-Command -Session $session -ScriptBlock {
            ...
            # Initialize array to store adapter configurations
            $adapterConfigs = @()
            ...
            foreach ($adapter in $adapters) {
                ...
                # Get all IPv4 addresses for this adapter
                $ipv4Addresses = Get-NetIPAddress -InterfaceIndex $adapter.ifIndex -AddressFamily IPv4
...
function Set-ServerDNSConfiguration {
    param (
        [string]$ComputerName,
        [PSCredential]$Credential,
        [string]$PrimaryDNS,
        [string]$SecondaryDNS = "1.1.1.1"  # Default to Cloudflare DNS as secondary
    )
    ...
    try {
        $session = New-RetryPSSession -ComputerName $ComputerName -Credential $Credential
        if (-not $session) {
            throw "Failed to establish session for DNS configuration"
        }
        ...
        Invoke-Command -Session $session -ScriptBlock {
            ...
            foreach ($adapter in $adapters) {
                ...
                Write-Host "DNS Servers configured for $($adapter.Name): $($dnsServers -join ', ')" -ForegroundColor Green
            }
        } -ArgumentList $PrimaryDNS, $SecondaryDNS
...
try {
    ...
    Write-Host "--------------------------------"
    foreach ($adapter in $networkStatus) {
        Write-Host "Adapter Name: $($adapter.AdapterName)"
        Write-Host "Current IP Address: $($adapter.IPAddress)"
        Write-Host "Desired IP Address: $DC2IP"
        Write-Host "Subnet Mask Length: $($adapter.PrefixLength)"
        Write-Host "Default Gateway: $($adapter.Gateway)"
        Write-Host "Current DNS Servers: $($adapter.DNSServers -join ', ')"
        Write-Host "Required Primary DNS: $DC1IP"
        Write-Host "DHCP Status: $(if($adapter.IsDHCPEnabled){'Enabled'}else{'Disabled (Static)'})"
        Write-Host "--------------------------------"
    }

    # DNS Configuration
    Write-Host "`nDNS Configuration is critical for domain join." -ForegroundColor Yellow
    Write-Host "Primary DNS will be set to DC1 ($DC1IP)" -ForegroundColor Yellow
    Write-Host "Secondary DNS will be set to 1.1.1.1 for backup resolution" -ForegroundColor Yellow

    $configureDNS = Read-Host "Do you want to configure DNS settings now? (yes/no)"
    if ($configureDNS -eq "yes") {
        $dnsResult = Set-ServerDNSConfiguration -ComputerName $TargetServer -Credential $LocalAdminCred -PrimaryDNS $DC1IP
        
        if (-not $dnsResult) {
            Write-Host "Failed to configure DNS settings. Please verify network connectivity and try again." -ForegroundColor Red
            exit 1
        }
        ...
        $verifyParams = @{
            ...
            ScriptBlock = {
                ...
                foreach ($adapter in $adapters) {
                    $dnsServers = (Get-DnsClientServerAddress -InterfaceIndex $adapter.ifIndex -AddressFamily IPv4).ServerAddresses
                    Write-Host "Adapter $($adapter.Name) DNS Servers: $($dnsServers -join ', ')"
                }
            }
        }
        Invoke-Command @verifyParams
    }

    # IP Configuration
    foreach ($adapter in $networkStatus) {
        if (-not $adapter.IsDHCPEnabled) {
...
Enter choice (1-3)"
            ...
            switch ($choice) {
                "1" {
                    if ($adapter.IPAddress -ne $DC2IP) {
                        Write-Host "WARNING: Current IP ($($adapter.IPAddress)) differs from desired DC2 IP ($DC2IP)" -ForegroundColor Red
                ...
                "2" {
                    ...
                    $dhcpParams = @{
                        ...
                        ArgumentList = $adapter.AdapterIndex
                    }
