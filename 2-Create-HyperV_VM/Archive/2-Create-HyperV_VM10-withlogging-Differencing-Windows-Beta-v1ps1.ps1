...
$Params = @{
    DismountVHDX = @{
        VHDXPath = "D:\VM\Setup\VHDX\Win11_23H2_English_x64_May_19_2024-100GB-unattend-Professional.VHDX"
    }
    NewCustomVMWithDifferencingDisk = @{
        VMName               = "$VMNamePrefix`_$Datetime"
        VMFullPath           = "$VMPath\$VMName"
        ParentVHDPath        = "D:\VM\Setup\VHDX\Win11_23H2_English_x64_May_19_2024-test3.VHDX"
        DifferencingDiskPath = "$VMFullPath\$VMName-diff.vhdx"
...
function Get-NextVMNamePrefix {
    ...
    return '{0:D3} - Win11-UnattendXML-TEST -' -f $nextPrefixNumber
}
