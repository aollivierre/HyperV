...
function Export-WACVMVirtualMachine {
    ...
    function main([string]$vmId, [string]$exportPath) {
        ...
        if ($err) {
            Write-Host "Couldn't retrieve the virtual machine. Error: $err" -ForegroundColor Red
            return
        }
        if ($vm) {
            $err = $null
            ...
            if ($err) {
                Write-Host "Couldn't export the virtual machine. Error: $err" -ForegroundColor Red
                return
            }
            Write-Host "Exported VM: $vmName to $exportPath" -ForegroundColor Green
        }
    }
    ...
    try {
        ...
        if ($module) {
            main -vmId $vmId -exportPath $exportPath
        } else {
