...
function Update-HostsFile {
    ...
    # Invoke-Command to update the hosts file using PowerShell 7 with admin credentials
...
function Execute-RemoteScript {
    ...
    Update-HostsFile -remoteHost $remoteHost -adminCredential $adminCredential -scriptContent1 $scriptContent1 -hostEntries $hostEntries

    # Create RDP sessions using standard user credentials
    Create-RDPSessionsRemote -remoteHost $remoteHost -standardCredential $standardCredential -scriptContent2 $scriptContent2 -hostEntries $hostEntries
}
