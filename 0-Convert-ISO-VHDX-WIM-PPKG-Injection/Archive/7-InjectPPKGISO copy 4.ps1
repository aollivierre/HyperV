...
function Copy-Ppkg {
    ...
    try {
        ...
        if (-Not (Test-Path -Path $provisioningDirectory)) {
            Write-Host "Creating Provisioning directory: $provisioningDirectory"
            New-Item -ItemType Directory -Path $provisioningDirectory -Force
        }

        Write-Host "Copying PPKG to Provisioning directory"
...
function Create-NewIso {
    ...
    try {
        ...
        & $oscdimgPath -m -o -u2 -udfver102 -b"$bootSectorFile" $WorkingDirectory $OutputIsoPath -ErrorAction Stop
    } catch {
